...
(() => {
  ...
  const TOPO = [
    ...
    ['Hotelaria', RAIZ + 'hotelaria/'], ['Simuladores', RAIZ + 'simuladores/'],
    ['Blog', RAIZ + 'blog/'], ['Contato', RAIZ + 'contato/'],
  ];

  const mapaBtn = document.createElement('button');
  mapaBtn.className = 'mapa-btn';
  mapaBtn.setAttribute('aria-label', 'Abrir resumo do site — mapa de procedimentos');
  mapaBtn.innerHTML = '<svg viewBox="0 0 16 16"><rect x="5.5" y="1" width="5" height="3.6"/><rect x="1" y="11.4" width="5" height="3.6"/><rect x="10" y="11.4" width="5" height="3.6"/><path d="M8 4.6v3M3.5 11.4V7.6h9v3.8"/></svg><span class="lbl">Resumo</span>';
  ...
  const montarMapa = (dados) => {
    ...
    el.setAttribute('aria-label', 'Resumo do site');
    el.innerHTML = `
      <button class="mapa-close" aria-label="Fechar resumo">✕</button>
      <div class="mapa-inner">
        <p class="overline">Resumo do site</p>
        <h2 class="mt-2">Tudo o que fazemos, em <em>um lugar</em>.</h2>
  ...
  mapaBtn.addEventListener('click', async () => {
    if (!mapaEl) {
      try {
        const resp = await fetch(RAIZ + 'data/especialidades.json');
  ...
  const marcarVazio = (img) => {
    ...
    const rotulo = (img.getAttribute('alt') || 'Fotografia').split('—')[0].trim();
