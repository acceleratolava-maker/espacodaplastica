...
(() => {
  ...
  document.querySelectorAll('form[data-lead]').forEach(form => {
    form.addEventListener('submit', (ev) => {
      ...
      if (form.dataset.leadMode === 'b2b') {
        ...
        msg = `Hello! I am ${nome || 'a physician'}${interesse ? ` (${interesse})` : ''} and I would like to learn about the hospital facilities for performing procedures.`;
        if (obs) msg += ` ${obs}`;
      } else {
        msg = `Hello! I am ${nome || 'a patient coming from the website'} and I would like to schedule a consultation`;
        if (interesse) msg += ` about ${interesse}`;
        msg += '.';
        if (obs) msg += ` ${obs}`;
      }
      window.open(`https://api.whatsapp.com/send?phone=${WA_PHONE}&text=${encodeURIComponent(msg)}`, '_blank', 'noopener');
  ...
  const RAIZ = '/en/';
  const GRUPO_ORDEM = ['Breasts', 'Body', 'Face', 'Intimate', 'Hair', 'Non-Surgical', 'Technology'];
  const TOPO = [
    ['The Hospital', RAIZ + 'estrutura/'], ['Medical Team', RAIZ + 'medicos/'], ['Safety', RAIZ + 'seguranca/'],
    ['Hospitality', RAIZ + 'hotelaria/'], ['Simulators', RAIZ + 'simuladores/'],
    ['Blog', RAIZ + 'blog/'], ['Contact', RAIZ + 'contato/'],
  ];

  const mapaBtn = document.createElement('button');
  mapaBtn.className = 'mapa-btn';
  mapaBtn.setAttribute('aria-label', 'Open the site overview — procedure map');
  mapaBtn.innerHTML = '<svg viewBox="0 0 16 16"><rect x="5.5" y="1" width="5" height="3.6"/><rect x="1" y="11.4" width="5" height="3.6"/><rect x="10" y="11.4" width="5" height="3.6"/><path d="M8 4.6v3M3.5 11.4V7.6h9v3.8"/></svg><span class="lbl">Overview</span>';
  ...
  const montarMapa = (dados) => {
    ...
    el.setAttribute('aria-label', 'Site overview');
    el.innerHTML = `
      <button class="mapa-close" aria-label="Close overview">✕</button>
      <div class="mapa-inner">
        <p class="overline">Site overview</p>
        <h2 class="mt-2">Everything we do, in <em>one place</em>.</h2>
  ...
  mapaBtn.addEventListener('click', async () => {
    if (!mapaEl) {
      try {
        const resp = await fetch('/data/especialidades-en-names.json');
  ...
  const marcarVazio = (img) => {
    ...
    const rotulo = (img.getAttribute('alt') || 'Photograph').split('—')[0].trim();
